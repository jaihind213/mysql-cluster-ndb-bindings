...
public:
  ...
  %ndbexception("NdbApiException") {
    $action
      if (result==NULL) {
        NDB_exception(NdbApiException,"NdbOperation object is NULL");
      }
  }
...
%extend NdbScanFilter {
  ...
  int cmp(BinaryCondition cond, int ColId, const char * BYTE, size_t len) {
    ...
    if (cond == NdbScanFilter::COND_EQ) {
      ...
      if (cmpLength == 0)
        return -1;
      stringVal = ndbFormatString(theColumn,BYTE,len);
      if (stringVal == NULL)
        return -1;
    } else if ((cond == NdbScanFilter::COND_LIKE)
               || (cond == NdbScanFilter::COND_NOT_LIKE)) {
      cmpLength = (Uint32)len;
      stringVal = (char *)malloc(len);
      memcpy(stringVal,BYTE,len);
  ...
  int cmpString(BinaryCondition cond, int ColId,
    ...
    if (cond == NdbScanFilter::COND_EQ) {
      ...
      if (cmpLength == 0)
        return -1;
      stringVal = ndbFormatString(theColumn,anInputString,len);
      if (stringVal == NULL)
        return -1;
    } else if ((cond == NdbScanFilter::COND_LIKE)
               || (cond == NdbScanFilter::COND_NOT_LIKE)) {
      cmpLength = (Uint32)len;
      stringVal = (char *)malloc(len);
      memcpy(stringVal,anInputString,len);
    }
