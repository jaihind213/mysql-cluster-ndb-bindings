...
public:
  ...
  ndb_mgm_handle(const char * connectString = 0)
  {
   ...
   * @return                -1 on error.
   */
  int connect( const char * connectString,int no_retries,
               int retry_delay_in_seconds, bool verbose) {
   ...
   */
  %newobject getConnectString;
  const char * getConnectString() {
    int stringLength = 256;
    char * theString = (char *)malloc(stringLength);
    return ndb_mgm_get_connectstring($self, theString, stringLength);
  }
  ...
  BackupWrapper * startBackup(BackupStartOption wait_completed) {
    ...
    BackupWrapper * reply_wrapper =
      (BackupWrapper *)malloc(sizeof(BackupWrapper));
   ...
   */
  int stop(int no_of_nodes, const int * node_list, bool abort=false) {
    return ndb_mgm_stop2($self,no_of_nodes,node_list,abort);
  }
  int stopNode(int node_id, bool abort=false) {
    int node_list[1];
    node_list[0]=node_id;
    return ndb_mgm_stop2($self,1,(const int *)node_list,abort);
  }
  ...
  int stop(int no_of_nodes, const int * node_list,
           bool abort, int *disconnect) {
    return ndb_mgm_stop3($self,no_of_nodes,node_list,abort,disconnect);
  }
  int stopNode(int node_id,
               bool abort, int *disconnect) {
    int node_list[1];
    node_list[0]=node_id;
    return ndb_mgm_stop3($self,1,(const int *)node_list,abort,disconnect);
  }
   ...
   * @return                Number of nodes stopped (-1 on error).
   */
  int restart(int no_of_nodes,const int * node_list, bool initial=false,
              bool nostart=false, bool abort=false) {
    return ndb_mgm_restart2($self, no_of_nodes, node_list, initial,
                            nostart, abort);
  }
  int restartNode(int node_id, bool initial=false,
                  bool nostart=false, bool abort=false) { 
    int node_list[1];
    node_list[0]=node_id;
    return ndb_mgm_restart2($self, 1, (const int *)node_list,
                            initial, nostart,abort);
  }
  ...
  int restart(int no_of_nodes, const int * node_list, bool initial,
              bool nostart, bool abort, int *disconnect) {

    return ndb_mgm_restart3($self, no_of_nodes, node_list, initial, nostart,
                            abort, disconnect);
  }
  int restartNode(int node_id, bool initial,
                  bool nostart, bool abort, int *disconnect) { 
    int node_list[1];
    node_list[0]=node_id;
    return ndb_mgm_restart3($self, 1, (const int *)node_list,
                            initial, nostart,abort, disconnect);
  }
  ...
  int startNode(int node_id) { 
    int node_list[1];
    node_list[0]=node_id;
    return ndb_mgm_start($self, 1, (const int *)node_list);
  }
  ...
  int setClusterlogSeverityFilter(ndb_mgm_event_severity severity,
    ...
    int ret = ndb_mgm_set_clusterlog_severity_filter($self, severity,
                                                     enable, reply);
  ...
  ndb_mgm_severity* getClusterLogSeverityFilter() {
    ...
    int ret =
      ndb_mgm_get_clusterlog_severity_filter($self,
                                             theSeverity,
  ...
  ndb_mgm_loglevel * getClusterlogLoglevel() {
    ...
    int ret = ndb_mgm_get_clusterlog_loglevel($self, theLoglevel,
                                              MGM_LOGLEVELS);
    if (ret == -1) {
      theLoglevel = NULL;
    }
  ...
  NdbLogEventManager *
    createNdbLogEventManager(const std::vector<NdbFilterItem> filter) {

    int * theFilter = (int *)malloc(sizeof(int)*(filter.size()*2+1));
    ...
    NdbLogEventManager* ret= new NdbLogEventManager(ndb_mgm_create_logevent_handle($self,
                                                                 theFilter));
    free(theFilter);
    return ret;
  }
  ...
  NdbLogEventManager * createNdbLogEventManager(const int * filter) {
    ...
    return new NdbLogEventManager(ndb_mgm_create_logevent_handle($self,
                                                                 filter));
  }
